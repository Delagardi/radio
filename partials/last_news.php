<div class="ui segment padded widget last-news">
    <div class="widget-head ui dividing header">
        <h1>Last news</h1>
        <div class="submenu">
            <a class="submenu-link active" href="#">All</a>
            <a class="submenu-link" href="#">Creative</a>
            <a class="submenu-link" href="#">Technology</a>
            <a class="submenu-link" href="#">World</a>
            <div class="submenu-nav">
                <i class="caret square left outline icon"></i>
                <i class="caret square right outline icon"></i>
            </div>
        </div>
    </div>
    <div class="widget-body">
        <div class="ui stackable two column grid">
            <div class="sixteen wide tablet eight wide computer column main-new">
                <div class="ui fluid image">
                    <div class="ui primary label">life style</div>
                    <div class="ui placeholder" style="width:100%; height:318px;">
                        <div class="image"></div>
                    </div>
                    <img class="lazy-load" data-src="<?= url('images/news/main_new.png') ?>">
                </div>
                <div class="content">
                    <div class="meta stats ui segment basic">
                        <div class="date">
                            <i class="calendar icon"></i>
                            <span>Apr 9, 2019</span>
                        </div>
                        <div class="popularity">
                            <div class="item">
                                <i class="comments icon"></i>
                                <span>2</span>
                            </div>
                            <div class="item">
                                <i class="thumbs up icon"></i>
                                <span>2</span>
                            </div>
                            <div class="item">
                                <i class="eye icon"></i>
                                <span>12</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="content">
                    <h2>Budget issues force the Tour to be cancelled at the central.</h2>
                    <p>
                        Stay focused and remember we design the best News and Magazine Themes.
                        It’s the ones closest to you that want to enviroments on a day basic to manifest
                        watch your back, but more importantly when you’re out the shower....
                    </p>
                    <button class="large ui primary button">Read More</button>
                </div>
            </div>
    
            <div class="sixteen wide tablet eight wide computer column">
                <div class="ui items unstackable short-news">
                    <div class="item">
                        <div class="image">
                            <div class="ui primary mini label">foods</div>
                            <div class="ui placeholder" style="height:72px;">
                                <div class="image"></div>
                            </div>
                            <img class="lazy-load" data-src="<?= url('images/news/new1.png') ?>">
                        </div>
                        <div class="content">
                            <div class="meta stats">
                                <div class="date">
                                    <i class="calendar icon"></i>
                                    <span>Apr 9, 2019</span>
                                </div>
                                <div class="popularity">
                                    <div class="item">
                                        <i class="comments icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="thumbs up icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="eye icon"></i>
                                        <span>12</span>
                                    </div>
                                </div>
                            </div>
                            <h5>Why the breakfast is the most important food for the humans?</h5>
                        </div>
                    </div>
                    
                    <div class="item">
                        <div class="image">
                            <div class="ui primary mini label">tech</div>
                            <div class="ui placeholder" style="height:72px;">
                                <div class="image"></div>
                            </div>
                            <img class="lazy-load" data-src="<?= url('images/news/new3.png') ?>">
                        </div>
                        <div class="content">
                            <div class="meta stats">
                                <div class="date">
                                    <i class="calendar icon"></i>
                                    <span>Apr 9, 2019</span>
                                </div>
                                <div class="popularity">
                                    <div class="item">
                                        <i class="comments icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="thumbs up icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="eye icon"></i>
                                        <span>12</span>
                                    </div>
                                </div>
                            </div>
                            <h5>The actual situation at the new techologies in 2019.</h5>
                        </div>
                    </div>

                    <div class="item">
                        <div class="image">
                            <div class="ui primary mini label">travel</div>
                            <div class="ui placeholder" style="height:72px;">
                                <div class="image"></div>
                            </div>
                            <img class="lazy-load" data-src="<?= url('images/news/new4.png') ?>">
                        </div>
                        <div class="content">
                            <div class="meta stats">
                                <div class="date">
                                    <i class="calendar icon"></i>
                                    <span>Apr 9, 2019</span>
                                </div>
                                <div class="popularity">
                                    <div class="item">
                                        <i class="comments icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="thumbs up icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="eye icon"></i>
                                        <span>12</span>
                                    </div>
                                </div>
                            </div>
                            <h5>The top 10 best countries to enjoy architecture in builgings.</h5>
                        </div>
                    </div>

                    <div class="item">
                        <div class="image">
                            <div class="ui primary mini label">racing</div>
                            <div class="ui placeholder" style="height:72px;">
                                <div class="image"></div>
                            </div>
                            <img class="lazy-load" data-src="<?= url('images/news/new2.png') ?>">
                        </div>
                        <div class="content">
                            <div class="meta stats">
                                <div class="date">
                                    <i class="calendar icon"></i>
                                    <span>Apr 9, 2019</span>
                                </div>
                                <div class="popularity">
                                    <div class="item">
                                        <i class="comments icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="thumbs up icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="eye icon"></i>
                                        <span>12</span>
                                    </div>
                                </div>
                            </div>
                            <h5>Design the best car model, for real competitions in USA.</h5>
                        </div>
                    </div>

                    <div class="item">
                        <div class="image">
                            <div class="ui primary mini label">life style</div>
                            <div class="ui placeholder" style="height:72px;">
                                <div class="image"></div>
                            </div>
                            <img class="lazy-load" data-src="<?= url('images/news/new5.png') ?>">
                        </div>
                        <div class="content">
                            <div class="meta stats">
                                <div class="date">
                                    <i class="calendar icon"></i>
                                    <span>Apr 9, 2019</span>
                                </div>
                                <div class="popularity">
                                    <div class="item">
                                        <i class="comments icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="thumbs up icon"></i>
                                        <span>2</span>
                                    </div>
                                    <div class="item">
                                        <i class="eye icon"></i>
                                        <span>12</span>
                                    </div>
                                </div>
                            </div>
                            <h5>Enjoy the nature’s life style inside of the jungles and forest.</h5>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>